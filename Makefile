...
debug: clear-port-forwards kind-check
	skaffold run --profile=debug --port-forward -f internal/skaffold.yaml &

.PHONY: sync
sync: clear-port-forwards kind-check
	skaffold dev -f internal/skaffold.yaml

.PHONY: sync-debug
sync-debug: clear-port-forwards kind-check
	skaffold dev --profile=debug -f internal/skaffold.yaml
...
test: kind-check
	...
	skaffold verify -f internal/skaffold.yaml
	go test ./...
